...
def _make_episode(*, blog, num, title, user, root_collection):
...
def test_persistent_player_survives_navigation(page, staging_site):
    ...
    focus_ok = page.evaluate(
        "(sel) => { const el = document.activeElement; const c = document.querySelector(sel); return el === c || (el && el.tagName === 'H1' && c && c.contains(el)); }",
        content,
    )
